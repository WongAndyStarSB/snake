...
Snake::Snake(const Pos2D& init_pos, size_t snake_length)
    ...
    if (snake_length == 0) {
        Logger::log_and_throw("Snake::Snake invalid_argument: snake_length should be greater than 0");
    }
...
std::unique_ptr<Snake> Snake::copy() const {
    std::unique_ptr<Snake> new_snake = std::make_unique<Snake>(this->head->pos, this->snake_segments.size());
    ...
    for (int i = 0; i < this->snake_segments.size(); ++i) {
        new_snake->snake_segments[i]->change_values_as(*(this->snake_segments[i]));
    }
...
size_t Snake::get_segment_index_from_head(const size_t& offset_from_head) const {
    if (offset_from_head >= snake_segments.size()) {
        Logger::log("Snake::get_segment_index_from_head InvalidArgument: offset_from_head (value:"+std::to_string(offset_from_head) + ") is out of range (snake_segments.size():" + std::to_string(snake_segments.size()) + ")", WARNING_1);
    }
...
void Snake::snake_move(Vector2D direction, Vector2D next_direction) {
    if (this->snake_segments.empty()) {
        throw std::runtime_error("snake_segments should not be empty");
    }
    if (this->tail_index >= this->snake_segments.size()) {
        throw std::runtime_error("this->tail_index should be less than this->snake_segments.size()");
    }
    ...
    this->snake_segments[this->tail_index]->pos = this->head->pos + direction;
